...
```bash
claude mcp add -s user calc-mcp -- npx -y @coo-quack/calc-mcp@2.1.1
```
...
```json
{
  "mcpServers": {
    "calc-mcp": {
      "command": "npx",
      "args": ["-y", "@coo-quack/calc-mcp@2.1.1"]
    }
  }
}
```
...
```bash
npx -y @coo-quack/calc-mcp@2.1.1
```
...
```bash
npm install -g @coo-quack/calc-mcp@2.1.1
calc-mcp
```
